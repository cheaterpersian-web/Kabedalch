...
export default function TestRunner() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://api:3001'}/api/tests/templates`)
      .then((r) => r.json())
      .then((t) => setTemplate(t.find((x: any) => x.id === id)));
  }, [id]);

  const submit = async () => {
    const r = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://api:3001'}/api/tests/${id}/submit`, {
